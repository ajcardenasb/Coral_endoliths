...
#pdf("outputs/proka_cfix_metabolism_btwSpecies_1V2.pdf", width = 5.5, height = 3, pointsize = 10)
...
## Ascorbate peroxidase K00434
cnts_t=as.data.frame(t(cnts_clr))
cnts_t$Species=map$Species[match(rownames(cnts_t), rownames(map))]
cnts_t$Treatment=map$Treatment[match(rownames(cnts_t), rownames(map))]
cnts_t$Group=paste(map$Species, map$Tissue, sep = "_")[match(rownames(cnts_t), rownames(map))]
cnts_cnt=subset(cnts_t, Treatment == "Control" )
boxplot(cnts_cnt$K00434 ~cnts_cnt$Group, main = "Reductive citrate cycle\n(Arnon-Buchanan cycle)", col=c("#8f2d56","#218380"), ylab = "clr-trasformed counts", xlab = "")
