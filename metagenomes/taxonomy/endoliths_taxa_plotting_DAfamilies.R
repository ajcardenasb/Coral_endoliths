...
clr_sig=subset(cnts_clr_sort, rownames(cnts_clr_sort) %in%  rownames(subset(ancom_band, Effect_size_W > 5)) |  rownames(cnts_clr_sort) %in%  rownames(subset(ancom_skel,  Effect_size_W > 5)) ) 
colour_taxa=data.frame(domain=tax$Domain[match(rownames(clr_sig),rownames(tax))])
#rownames(clr_sig)=paste(rownames(clr_sig), tax$Domain[match(rownames(clr_sig),rownames(tax))], sep = " -" )
...
# replicated heatmaps Delta and Cyano
kai=read.table("Input_files/Endoliths_kaiju_taxonomy_500", sep = "\t", header = T, fill = T, quote = "")
del_cyan=subset(kai, Phylum == "Cyanobacteria" | Class == "Deltaproteobacteria")$label
clr_del_cyan=subset(cnts_clr_sort, rownames(cnts_clr_sort) %in%  del_cyan)
colour_taxa=data.frame(domain=kai$Phylum[match(rownames(clr_del_cyan),kai$label)])
rownames(colour_taxa)=rownames(clr_del_cyan)
pdf("outputs/heatmaps_ANCOMBC_delta_cyano.pdf", height = 12, width = 9)
pheatmap(clr_del_cyan, color = colorRampPalette(c("#1f77b4", "white", "#f94144"))(50),  angle_col = "0",  scale = "row", cluster_row= T, cluster_col = FALSE, border_color = NA,  cellwidth = 5, cellheight =  5, fontsize_row = 6, fontsize_col= 8, legend = F,  annotation_row = colour_taxa,  gaps_col = c(6,12,18), annotation_colors =  list(domain =c(Proteobacteria="#31AF6C", Cyanobacteria="#F38C16") ))
dev.off()
